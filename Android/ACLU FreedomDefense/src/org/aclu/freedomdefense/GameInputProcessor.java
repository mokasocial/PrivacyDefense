...
public class GameInputProcessor implements InputProcessor {
	...
	public boolean touchDown(int x, int y, int pointer, int button) {
		...
		if (Game.instance.buildMode) {
			...
			if (x >= Game.START_RECT.x && x <= Game.START_RECT.x + Game.START_RECT.width && (Game.screenHeight - y) >= Game.START_RECT.y && (Game.screenHeight - y) <= Game.START_RECT.y + Game.START_RECT.height) {
		...
		if (Game.instance.isPaused) {
			...
			if (x >= Game.RESTART_RECT.x && x <= Game.RESTART_RECT.x + Game.RESTART_RECT.width && (Game.screenHeight - y) >= Game.RESTART_RECT.y && (Game.screenHeight - y) <= Game.START_RECT.y + Game.RESTART_RECT.height) {
		...
		if (Game.instance.runningDrd && !Game.instance.buildMode && x >= Game.DRD_PAUSE_RECT.x && x <= Game.DRD_PAUSE_RECT.x + Game.DRD_PAUSE_RECT.width && (Game.screenHeight - y) >= Game.DRD_PAUSE_RECT.y && (Game.screenHeight - y) <= Game.DRD_PAUSE_RECT.y + Game.DRD_PAUSE_RECT.height) {
		...
		if (!Game.instance.isPaused && !Game.instance.buildMode) {
			...
			for (int i = 0; i < Game.instance.towers.size(); i++) {
				...
				if (Game.instance.towers.get(i).m_x == gameCoordX && Game.instance.towers.get(i).m_y == gameCoordY) {
			...
			if (Game.instance.selected != null && x >= Game.START_RECT.x && x <= Game.START_RECT.x + Game.START_RECT.width && (Game.screenHeight - y) >= Game.START_RECT.y && (Game.screenHeight - y) <= Game.START_RECT.y + Game.START_RECT.height) {
		...
		if (!Game.instance.isPaused && Game.instance.life > 0) {
			for (TowerType towerType : Game.instance.free_towers) {
				if (x <= Game.instance.uiPanelWidth && x >= 35) {
					if (y >= (30) && y <= (60)) {
						// Check to make sure there is enough money to buy the
						// tower.
						if (Game.instance.money >= Game.instance.free_towers.get(0).getPrice()) {
						...
						if (Game.instance.money >= Game.instance.free_towers.get(1).getPrice()) {
						...
						if (Game.instance.money >= Game.instance.free_towers.get(2).getPrice()) {
					 ...
					 * Game.instance.cursorState =
					 * Game.instance.free_towers.get(3);
					 * Game.instance.cursorLocX = x; Game.instance.cursorLocY =
					 * y; }
					 */

					if (Game.instance.cursorState != null) {
						Game.instance.cursorTexture = new TextureRegion(Game.instance.cursorState.getTextureRegion(), 0, 0, 32, 32);
					}
				}
			}
		}
	...
	public boolean touchUp(int x, int y, int pointer, int button) {
		...
		if (Game.instance.cursorState != null) {
			...
			if (!tileContainsTower(gameCoordX, gameCoordY) && !(Game.instance.tiles[gameCoordX][gameCoordY] >= 5 && Game.instance.tiles[gameCoordX][gameCoordY] <= 10)) {
