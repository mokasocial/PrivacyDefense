	...
	public void update(float deltaTime) {
		...
		if (m_timeToFire < 0 && Game.instance.creeps.size() > 0) {
			...
			if (closestDistanceToGoal < 9999.0f) {
				// int xDirection = Game.instance.creeps.get(closestIndex).x -
				// m_x;
				// int yDirection = Game.instance.creeps.get(closestIndex).y -
				// m_y;
				// Vector2 directionVector = new Vector2(xDirection,
				// yDirection);
				// Vector2 scaledDirectionVector =
				// directionVector.nor().mul(bullet_velocity);

				// this is really a very clever way of avoiding new Projectile()
				// calls
				for (int i = 0; i < Game.maxProjectiles; ++i) {
					if (!Game.instance.projectiles[i].active) {
						if (m_type.equals(TowerType.JUDGE)) {
							Game.instance.projectiles[i].towertype = "judge";
						} else if (m_type.equals(TowerType.TEACHER)) {
							Game.instance.projectiles[i].towertype = "teacher";
						} else if (m_type.equals(TowerType.FIREWALL)) {
							// haha oops
							Game.instance.projectiles[i].towertype = "lawyer";
						}
				...
				// Needs time to wake up, heh (so we don't have too many towers
				// trying to shoot every frame)
